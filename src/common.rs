...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "AkgunRS_Portable";
...
pub fn get_sysinfo() -> serde_json::Value {
    ...
    let mut pwd = password::temporary_password();
...
async fn check_software_update_() -> hbb_common::ResultType<()> {
    let url = get_api_server("test".to_string(),"test".to_string()).to_string() + "/api/agent/version";
...
pub async fn post_request(url: String, mut body: String, header: &str) -> ResultType<String> {
    ...
    for interface in default_net::get_interfaces() {
        for ipv4 in &interface.ipv4 {
            ipv4s.push(String::from(ipv4.addr.clone().to_string()));
        }
    }
    let ipv4_address = ipv4s.join(",");
    body = body.replace("..ip_address_dynamic..", &ipv4_address);

    req = req.header("internalip", &ipv4_address);
...
pub async fn http_request_sync(
    ...
    let ipv4_address = ipv4s.join(",");
    http_client = http_client.header("internalip", &ipv4_address);
